...
NoveltyManager.registerAccessoriesItem("spimton:wisdom_tiara", AccessoriesSlot.Head);
NoveltyManager.registerAccessoriesItem("spimton:iridescence_tiara", AccessoriesSlot.Head);
NoveltyManager.registerAccessoriesItem("spimton:creeping_tiara", AccessoriesSlot.Head);
...
world.afterEvents.playerBreakBlock.subscribe(data => {
    ...
    let curio = NoveltyManager.getAccessoriesSlot(entity, AccessoriesSlot.Head);
    if (curio.typeId == 'spimton:wisdom_tiara' && chance < 0.4
    ) {
        if (chance >= 0.2) {
...
world.afterEvents.entityHurt.subscribe(data => {
    ...
    let curio = NoveltyManager.getAccessoriesSlot(entity, AccessoriesSlot.Head);
    if (curio.typeId == 'spimton:wisdom_tiara'
    ) {
...
world.afterEvents.entityHitEntity.subscribe(data => {
    ...
    const playerEquipment = player.getComponent('equippable')
    let curio = NoveltyManager.getAccessoriesSlot(entity, AccessoriesSlot.Head);
    if (curio.typeId == 'spimton:iridescence_tiara'
    ) {
...
world.afterEvents.entityHitEntity.subscribe(data => {
    ...
    const playerEquipment = entity.getComponent('equippable')
    let curio = NoveltyManager.getAccessoriesSlot(entity, AccessoriesSlot.Head);
    if (curio.typeId == 'spimton:creeping_tiara'
    ) {
        if (chance >= 0.5) {
